    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          <Text style={styles.headerTitle}>Reclamo: Orden #{claim.orderId.slice(0, 8)}</Text>
          <Text style={styles.headerSubtitle}>Estado: {claim.status === 'open' ? 'Pendiente' : claim.status === 'resolved' ? 'Válido' : 'Inválido'}</Text>
        </View>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={[styles.messageBubble, isAdmin ? styles.messageAdmin : styles.messageUser]}>
              <Text style={styles.messageSender}>{isAdmin ? 'Administrador' : 'Usuario'}</Text>
              <Text style={[styles.messageText, isAdmin && { color: '#fff' }]}>{item.content}</Text>
              <Text style={[styles.messageTime, isAdmin && { color: 'rgba(255,255,255,0.7)' }]}>{formatDateTime(item.createdAt)}</Text>
            </View>
          );
        }}
        ListEmptyComponent={<Text style={{ textAlign: 'center', marginTop: 20, color: Colors.textMuted }}>No hay mensajes aún.</Text>}
      />
      ...
      {claim.status === 'open' ? (
        ...
        <View style={styles.closedContainer}>
          <Text style={styles.closedText}>El reclamo ha sido cerrado.</Text>
        </View>
      )}
    </KeyboardAvoidingView>
  );
